...
class HeuristicNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'HeuristicNode initialized.')
    ...
    def computer_and_publish_path(self, items):
        try:
            ...
            # Compute a greedy approximation to the TSP using A* segments

            if not goal_poses:
                self.get_logger().warn('No goal poses to plan.')
                return

            self.get_logger().info(f'Computing greedy path for {len(goal_poses)} goal poses. This may take some time...')

            # greedy order and path computation
            remaining = list(range(len(goal_poses)))
            ordered_indices = []

            # total path accumulator
            total_path = Path()
            ...
            while remaining:
                ...
                # evaluate cost to each candidate index
                for idx in remaining:
                    candidate = goal_poses[idx]
                    seg_path, seg_cost = self.compute_path_segment(current_pose, candidate)
                    if seg_path is not None and seg_cost < best_seg_cost:
                        best_idx = idx
                        best_seg = seg_path
                        best_seg_cost = seg_cost

                # if we couldn't find a valid next segment, planning failed
                if best_idx is None:
                    return None, float('inf'), None

                # append best candidate index to order and extend the path
                ordered_indices.append(best_idx)

                if total_path.poses and best_seg.poses:
                    total_path.poses.extend(best_seg.poses[1:])
                else:
                    total_path.poses.extend(best_seg.poses)
            ...
            if best_path:
                self.get_logger().info(f'Greedy path cost: {best_cost:.2f} for order: {best_order}')
            else:
                self.get_logger().error('Greedy planner failed to produce a valid path.')
